...
function findTonnage(text) {
  ...
  const labelMatch = source.match(/(?:提货吨位|吨位|数量|重量)\s*[:：\-\s]*([0-9]+(?:\.[0-9]+)?)/i);
  return labelMatch ? labelMatch[1] : '';
}

function findCargoCategory(text) {
  const source = String(text ?? '');
  if (source.includes('碳锰')) return '碳锰';
  if (source.includes('硅锰')) return '硅锰';
  if (source.includes('其它') || source.includes('其他')) return '其它';
  return '';
}
...
function inferCompany(text, fallback) {
  const source = String(text ?? '');
  const labelMatch = extractAfterLabel(source, ['提货公司', '公司', '单位', '客户', '提货单位', '发货单位', '收货单位']);
  ...
  for (const candidate of candidates) {
    if (candidate.length >= 3 && candidate.length <= 24 && /公司|贸易|物流|商贸|工贸|实业|能源|材料|工厂|集团/.test(candidate)) {
      return candidate;
    }
  }
